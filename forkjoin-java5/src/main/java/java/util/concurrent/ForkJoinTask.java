 ...
 * 
 * <p>
 * A "main" {@code ForkJoinTask} begins execution when submitted to a {@link ForkJoinPool}. Once
 ...
 * enforceable) reflecting their intended use as computational tasks calculating pure functions or
 ...
 * has been computed. Computations should avoid {@code synchronized} methods or blocks, and should
 * minimize other blocking synchronization apart from joining other tasks or using synchronizers
 * such as Phasers that are advertised to cooperate with fork/join scheduling. Tasks should also not
 * perform blocking IO, and should ideally access variables that are completely independent of those
 * accessed by other running tasks. Minor breaches of these restrictions, for example using shared
 * output streams, may be tolerable in practice, but frequent use may result in poor performance,
 * and the potential to indefinitely stall if the number of threads not waiting for IO or other
 * external synchronization becomes exhausted. This usage restriction is in part enforced by not
 * permitting checked exceptions such as {@code IOExceptions} to be thrown. However, computations
 * may still encounter unchecked exceptions, that are rethrown to callers attempting to join them.
 * These exceptions may additionally include {@link RejectedExecutionException} stemming from
 * internal resource exhaustion, such as failure to allocate internal task queues. Rethrown
 ...
 * {@link RecursiveAction} for computations that do not return results, or {@link RecursiveTask} for
 * those that do. Normally, a concrete ForkJoinTask subclass declares fields comprising its
 * parameters, established in a constructor, and then defines a {@code compute} method that somehow
 * uses the control methods supplied by this base class. While these methods have {@code public}
 * access (to allow instances of different task subclasses to call each other's methods), some of
 * them may only be called from within other ForkJoinTasks (as may be determined using method
 * {@link #inForkJoinPool}). Attempts to invoke them in other contexts result in exceptions or
 * errors, possibly including {@code ClassCastException}.
 * 
 * <p>
 ...
 * {@link Phaser}, {@link #helpQuiesce}, and {@link #complete}) that may be of use in constructing
...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
	 ...
	 * methods in ForkJoinWorkerThread and ForkJoinPool. The methods of this class are more-or-less
	 * layered into (1) basic status maintenance (2) execution and awaiting completion (3)
	 * user-level methods that additionally report results. This is sometimes hard to see because
	 * this file orders exported methods in a way that flows well in javadocs.
	 */
	 ...
	 * until completed, upon which status holds value NORMAL, CANCELLED, or EXCEPTIONAL. Tasks
	 * undergoing blocking waits by other threads have the SIGNAL bit set. Completion of a stolen
	 * task with SIGNAL set awakens any waiters via notifyAll. Even though suboptimal for some
	 * purposes, we use basic builtin wait/notify to take advantage of "monitor inflation" in JVMs
	 * that we would otherwise need to emulate to avoid adding further per-task bookkeeping
	 * overhead. We want these monitors to be "fat", i.e., not use biasing or thin-lock techniques,
	 * so use some odd coding idioms that tend to avoid them.
	 */
	...
	/**
	 * Marks completion and wakes up threads waiting to join this task, also clearing signal request
	 * bits.
	 * 
	 * @param completion
	...
	/**
	 * Tries to block a worker thread until completed or timed out. Uses Object.wait time argument
	 * conventions. May fail on contention or interrupt.
	 * 
	 * @param millis
	 *            if > 0, wait time.
	 */
	final void tryAwaitDone(final long millis) {
		int s;
		try {
	 ...
	 */
	private int externalInterruptibleAwaitDone(final long millis) throws InterruptedException {
		int s;
	...
	/**
	 * Table of exceptions thrown by tasks, to enable reporting by callers. Because exceptions are
	 * rare, we don't directly keep them with task objects, but instead use a weak ref table. Note
	 ...
	 * locking, and weak references for keys. The table has a fixed capacity because it only
	 * maintains task exceptions long enough for joiners to access them, so should never become very
	 * large for sustained periods. However, since we do not know when the last joiner completes, we
	 * must use weak references and expunge them. We do so on each operation (hence full locking).
	 * Also, some thread in any ForkJoinPool will call helpExpungeStaleExceptions when its pool
	 * becomes isQuiescent.
	 */
	...
	/**
	 * Returns a rethrowable exception for the given task, if available. To provide accurate stack
	 * traces, if the exception was not thrown by the current thread, we try to create a new
	 * exception of the same type as the one thrown, but with the recorded exception as its cause.
	 * If there is no such constructor, we instead try to use a no-arg constructor, followed by
	 * initCause, to the same effect. If none of these apply, or any fail due to other exceptions,
	 * we return the recorded exception, which is still correct, although it may contain a
	...
	private Throwable getThrowableException() {
		...
		if (e.thrower != Thread.currentThread().getId()) {
			...
			try {
				...
				if (noArgCtor != null) {
					final Throwable wx = (Throwable) (noArgCtor.newInstance());
					wx.initCause(ex);
					return wx;
				}
	...
	/**
	 * If lock is available, poll stale refs and remove them. Called from ForkJoinPool when pools
	 * become quiescent.
	 */
	...
	/**
	 * Report the result of invoke or join; called only upon non-normal return of internal versions.
	 */
	...
	/**
	 * Arranges to asynchronously execute this task. While it is not necessarily enforced, it is a
	 * usage error to fork a task more than once unless it has completed and been reinitialized.
	 * Subsequent modifications to the state of this task or any data it operates on are not
	 * necessarily consistently observable by any thread other than the one executing it unless
	 * preceded by a call to {@link #join} or related methods, or a call to {@link #isDone}
	 * returning {@code true}.
	 * 
	 * <p>
	 * This method may be invoked only from within {@code ForkJoinPool} computations (as may be
	 * determined using method {@link #inForkJoinPool}). Attempts to invoke in other contexts result
	 * in exceptions or errors, possibly including {@code ClassCastException}.
	 * 
	...
	/**
	 * Returns the result of the computation when it {@link #isDone is done}. This method differs
	 ...
	 * or throws an (unchecked) {@code RuntimeException} or {@code Error} if the underlying
	 * computation did so.
	 * 
	 * @return the computed result
	 */
	 ...
	 * tasks is not guaranteed upon exceptional return. The status of each task may be obtained
	 * using {@link #getException()} and related methods to check if they have been cancelled,
	 ...
	 * more than one task encounters an exception, then this method throws any one of these
	 * exceptions. If any task encounters an exception, others may be cancelled. However, the
	 * execution status of individual tasks is not guaranteed upon exceptional return. The status of
	 * each task may be obtained using {@link #getException()} and related methods to check if they
	 * have been cancelled, completed normally or exceptionally, or left unprocessed.
	 * 
	 * <p>
	 * This method may be invoked only from within {@code ForkJoinPool} computations (as may be
	 * determined using method {@link #inForkJoinPool}). Attempts to invoke in other contexts result
	 ...
	 * completed or could not be cancelled for some other reason. If successful, and this task has
	 * not started when {@code cancel} is called, execution of this task is suppressed. After this
	 * method returns successfully, unless there is an intervening call to {@link #reinitialize},
	 * subsequent calls to {@link #isCancelled}, {@link #isDone}, and {@code cancel} will return
	 * {@code true} and calls to {@link #join} and related methods will result in
	 * {@code CancellationException}.
	 * 
	 * <p>
	 * This method may be overridden in subclasses, but if so, must still ensure that these
	 * properties hold. In particular, the {@code cancel} method itself must not throw exceptions.
	 * 
	 * <p>
	 ...
	 *            this value has no effect in the default implementation because interrupts are not
	 *            used to control cancellation.
	 * 
	...
	/**
	 * Cancels, ignoring any exceptions thrown by cancel. Used during worker and pool shutdown.
	 * Cancel is spec'ed not to throw any exceptions, but if it does anyway, we have no recourse
	 * during shutdown, so guard against this case.
	 */
	 ...
	 * cancelled.
	 * 
	 * @return {@code true} if this task completed without throwing an exception and was not
	 *         cancelled
	 */
	...
	/**
	 * Completes this task abnormally, and if not already aborted or cancelled, causes it to throw
	 * the given exception upon {@code join} and related operations. This method may be used to
	 * induce exceptions in asynchronous tasks, or to force completion of tasks that would not
	 * otherwise complete. Its use in other situations is discouraged. This method is overridable,
	 * but overridden versions must invoke {@code super} implementation to maintain guarantees.
	 * 
	 * @param ex
	 *            the exception to throw. If this exception is not a {@code RuntimeException} or
	 *            {@code Error}, the actual exception thrown will be a {@code RuntimeException} with
	 *            cause {@code ex}.
	 */
	public void completeExceptionally(final Throwable ex) {
		setExceptionalCompletion((ex instanceof RuntimeException) || (ex instanceof Error) ? ex : new RuntimeException(
				ex));
	}

	/**
	 * Completes this task, and if not already aborted or cancelled, returning the given value as
	 * the result of subsequent invocations of {@code join} and related operations. This method may
	 * be used to provide results for asynchronous tasks, or to provide alternative handling for
	 * tasks that would not otherwise complete normally. Its use in other situations is discouraged.
	 * This method is overridable, but overridden versions must invoke {@code super} implementation
	 * to maintain guarantees.
	 * 
	 * @param value
	...
	public final V get() throws InterruptedException, ExecutionException {
		final int s = (Thread.currentThread() instanceof ForkJoinWorkerThread) ? doJoin()
				: externalInterruptibleAwaitDone(0L);
		Throwable ex;
		if (s == CANCELLED)
			throw new CancellationException();
		if (s == EXCEPTIONAL && (ex = getThrowableException()) != null)
	 ...
	 */
	public final V get(final long timeout, final TimeUnit unit) throws InterruptedException, ExecutionException,
			TimeoutException {
	 ...
	 * useful when processing collections of tasks when some have been cancelled or otherwise known
	 * to have aborted.
	 */
	 ...
	 * {@link ForkJoinPool#isQuiescent is quiescent}. This method may be of use in designs in which
	 * many tasks are forked, but none are explicitly joined, instead executing them until all are
	 * processed.
	 * 
	 * <p>
	 * This method may be invoked only from within {@code ForkJoinPool} computations (as may be
	 ...
	 * {@code getException()} reports {@code null}. However, the value returned by
	 * {@code getRawResult} is unaffected. To clear this value, you can invoke
	 * {@code setRawResult(null)}.
	 */
	...
	/**
	 * Returns the pool hosting the current task execution, or null if this task is executing
	...
	/**
	 * Tries to unschedule this task for execution. This method will typically succeed if this task
	 * is the most recently forked task by the current thread, and has not commenced executing in
	 * another thread. This method may be useful when arranging alternative local processing of
	 * tasks that could have been, but were not, stolen.
	 * 
	 * <p>
	 * This method may be invoked only from within {@code ForkJoinPool} computations (as may be
	 * determined using method {@link #inForkJoinPool}). Attempts to invoke in other contexts result
	...
	/**
	 * Returns an estimate of how many more locally queued tasks are held by the current worker
	 * thread than there are other worker threads that might steal them. This value may be useful
	 * for heuristic decisions about whether to fork other tasks. In many usages of ForkJoinTasks,
	 * at steady state, each worker should aim to maintain a small constant surplus (for example, 3)
	...
	/**
	 * Immediately performs the base action of this task. This method is designed to support
	 * extensions, and should not in general be called otherwise. The return value controls whether
	 * this task is considered to be done normally. It may return false in asynchronous actions that
	 * require explicit invocations of {@link #complete} to become joinable. It may also throw an
	 ...
	 * cannot be accessed without contention with other threads. This method is designed primarily
	...
	/**
	 * Unschedules and returns, without executing, the next task queued by the current thread but
	 * not yet executed. This method is designed primarily to support extensions, and is unlikely to
	 * be useful otherwise.
	 * 
	 * <p>
	 * This method may be invoked only from within {@code ForkJoinPool} computations (as may be
	...
	/**
	 * Unschedules and returns, without executing, the next task queued by the current thread but
	 * not yet executed, if one is available, or if not available, a task that was forked by some
	 * other thread, if available. Availability may be transient, so a {@code null} result does not
	 * necessarily imply quiescence of the pool this task is operating in. This method is designed
	...
	/**
	 * Adaptor for Runnables. This implements RunnableFuture to be compliant with
